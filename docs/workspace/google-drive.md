...
The pinned Kent v479 `bigBedNamedItems` utility queries checksum-bound local copies of the hg19/hg38 Common tracks, then local ClinVar tracks only for Common-unresolved identifiers. The default `bounded_local_v1` policy never contacts or stores the complete 65/68 GiB BigBeds. Remaining identifiers are explicit unresolved coverage, not dbSNP absence or reference genotypes. An 80% exact source-placement sanity threshold detects wrong assemblies or corrupted inputs; ordinary incompleteness above it permits selector-last publication.
