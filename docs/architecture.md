...
Normalization validates M1 manifests and checksums, then uses source-build-matched marker definitions, a checksummed local reference sequence (FAI-backed for large FASTA), and optional liftover mappings. M0B provisions these independently of genotype calls. Its common-first accelerator downloads identity-bound, resumable segments of each pinned common BigBed, queries an assembled local copy, and sends only the exact unresolved set to the complete remote index. Full-index subprocesses share one bounded executor but have stable, isolated UDC caches because Kent's sparse cache has no locking. Common absence is never scientific absence; only a completed full query establishes a non-return. Deterministic merge and the existing exact placement/SNV validation preserve scientific semantics regardless of completion order or worker count. The FASTA is checksum-verified, and selector-last publication remains fail-closed.
